...
# 图片路径要做修改
# DATA/ /mnt/bn/med-mllm-lfv2/ 

# 配置
INPUT_DIR = "/mnt/bn/med-mllm-lfv2/linjh/project/med_vlm_rl/code/0609_difficulty/input"
OUTPUT_DIR = "/mnt/bn/med-mllm-lfv2/linjh/project/med_vlm_rl/code/0609_difficulty/visualization"
...
def load_sampled_data():
    ...
    for file_path in json_files:
        ...
        dataset_name = filename.replace("_samples.json", "")
        ...
        try:
            ...
            print(f"已加载并处理 {dataset_name}: {len(processed_data)} 条数据")
...
def generate_html_visualization(data_files):
    ...
    # 为每个数据集生成单独的可视化文件
    for dataset_name, dataset_info in data_files.items():
        try:
            print(f"正在为 {dataset_name} 生成可视化...")
            
            # 创建临时JSON文件以供JsonVisualizer使用
            temp_json_file = os.path.join(OUTPUT_DIR, f"temp_{dataset_name}.json")
            
            # 限制显示的样本数量以提高性能
            display_data = dataset_info['data'][:100] if len(dataset_info['data']) > 100 else dataset_info['data']
            ...
            # 设置输出文件路径
            output_file = os.path.join(OUTPUT_DIR, f"{dataset_name}_visualization.html")
            ...
            JsonVisualizer.visualize(
                ...
                title=f"数据集可视化: {dataset_name} (显示前{len(display_data)}条)",
            ...
            print(f"为 {dataset_name} 生成可视化时出错: {e}")
            # 清理临时文件
            temp_json_file = os.path.join(OUTPUT_DIR, f"temp_{dataset_name}.json")
...
<body>
    <div class="container">
        ...
        <div class="note">
            <strong>说明:</strong> 本页面展示了从各个医疗数据集中采样的数据。每个数据集显示前100条记录的可视化结果。
            数据已经过处理，sharegpt格式的对话被转换为Q&A格式，图片路径已修正。
        </div>
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">平均每类样本数</div>
            </div>
        </div>
...
"""
    ...
    avg_samples = total_samples / total_datasets if total_datasets > 0 else 0
    ...
    for dataset_name, dataset_info in data_files.items():
        card_html = f"""
            <div class="dataset-card">
                <div class="dataset-header">{dataset_name}</div>
                <div class="dataset-content">
                    <div class="dataset-count">{dataset_info['count']} 条样本</div>
                    <p>来源文件: {os.path.basename(dataset_info['file_path'])}</p>
                    <a href="{dataset_name}_visualization.html" class="view-link">查看详细可视化</a>
                </div>
            </div>
        """
        dataset_cards.append(card_html)
...
def main():
    ...
    if not data_files:
        print("错误: 未找到采样数据文件")
        print(f"请确保先运行 0_sample_data.py 生成采样数据到 {INPUT_DIR}")
        return
